# DJI Batch LUTer

这是一个专门为大疆 (DJI) 全系列设备开发的视频批量转换工具。它可以将 Action、Mavic、Air、Pocket 等系列录制的 D-Log / D-Log M 原始素材，通过官方 LUT 快速还原为标准 Rec.709 色彩，并支持应用各类官方特定的调色滤镜。

目前支持的设备包括但不限于：
- **Action 系列**: Action 4, Action 5 Pro, Action 6
- **Pocket 系列**: Pocket 3, Pocket 4, Pocket 6
- **Mavic 系列**: Mavic 3, Mavic 4 Pro

![Main Interface](doc/ScreenShot_2026-04-19_173554_640.png)

## 🌟 核心功能

- **DJI 全系列色彩还原**:
  - **Action 系列**: 支持 Action 4/5 Pro 等 D-Log M 还原。
  - **无人机系列**: 支持 Mavic、Air、Mini 等系列的 D-Log / D-Log M 色彩还原。
- **官方调色滤镜支持**: 支持加载 DJI 官方发布的各类风格化调色滤镜（如 Forest, Ice, Nature 等）。
- **智能滤镜库管理**: 自动扫描 `config/` 目录，按设备系列（Action, Mavic, Air 等）和功能（还原/调色）自动分类。
- **全硬件加速**: 支持 NVIDIA (NVENC)、Intel (QSV) 和 AMD (AMF) 显卡加速，实现极速导出。**若显卡驱动不支持或硬件编码失败，程序将自动切换至 CPU 模式处理。**
- **并行处理**: 支持多任务同时并行转换，充分发挥硬件性能。
- **极简打包体积**: 通过 UPX 深度压缩及模块排除技术，生成的单文件 EXE 仅约 **31MB**，轻量且高效。
- **环境要求**:
  - **Python**: 3.13.3+
  - **FFmpeg**: 4.4.1 稳定版 (推荐使用内置脚本下载)
- **配置持久化**: 自动记忆路径设置、选中的设备、滤镜以及编码器偏好。
- **图形界面**: 简单易用的 PyQt6 交互界面，实时显示处理日志和进度。

## 📋 支持列表

| 设备系列                     | 还原滤镜 (Normalization) | 调色滤镜 (Color Grading)  | 说明                  |
| :----------------------- | :------------------- | :-------------------- | :------------------ |
| **Osmo Action 4**        | ✅ D-Log M to Rec.709 | ✅ Forest, Ice, Nature | 🎨 全系列官方调色支持           |
| **Osmo Action 5 Pro**    | ✅ D-Log M to Rec.709 | ✅ Ju, Lan, Mei, Zhu   | ✨ 最新 Pro 系列滤镜支持       |
| **Osmo Action 6**        | ✅ D-Log M to Rec.709 | -                     | 🚀 新增 Action 6 适配       |
| **Osmo Pocket 3**        | ✅ D-Log M to Rec.709 | ✅ Spring, Summer...  | 🌸 包含四季风格调色           |
| **Osmo Pocket 4**        | ✅ D-Log to Rec.709   | -                     | 🎞️ 包含标准及 Vivid 还原      |
| **Osmo Pocket 6**        | ✅ 目录已预留            | -                     | ⏳ 待补充滤镜文件             |
| **Mavic 3 系列**           | ✅ D-Log, D-Log M     | -                     | 🛸 包含标准及 Vivid 还原      |
| **Mavic 4 Pro**          | ✅ D-Log, D-Log M     | -                     | 🚁 包含标准及 Vivid 还原      |
| **其他设备**                 | 自动识别子目录              | 自动识别子目录               | 📂 支持通过 `config/` 目录扩展 |

## 📂 项目结构

```text
DJI/
├── config/               # 存放 DJI 官方及自定义 LUT 文件 (*.cube)
│   ├── Action 4/         # 按设备系列分类
│   │   ├── Normalization/ # 还原滤镜 (如 D-Log M to Rec.709)
│   │   └── Color Grading/ # 调色滤镜
│   ├── Action 5 Pro/
│   └── Mavic 3/          # 其他设备系列...
├── doc/                  # 项目说明文档
│   ├── QA.md             # 硬件加速常见问题 (Q&A)
│   └── packaging.md      # 项目打包指南
├── src/
│   ├── DJI_Batch_LUTer.py # 图形界面主程序
│   ├── setup_ffmpeg.py    # FFmpeg 自动配置脚本
│   ├── build.py           # 一键打包脚本
│   └── assets/            # 程序图标及 Logo 资源
├── RAW/                  # 默认原始素材存放目录
├── EXPORT/               # 默认导出视频存放目录
├── README.md             # 项目说明文档
├── requirements.txt      # Python 依赖包列表
├── config.example.json   # 配置文件模板
└── dji_luter_config.json # 自动生成的配置文件 (记住用户设置)
```

## 📖 详细文档

更多关于本工具的使用细节，请参阅：
- [硬件加速常见问题 (Q&A)](doc/QA.md)
- [项目打包指南](doc/packaging.md)

## 🚀 快速开始 (开发者/从源码运行)

如果你是开发者或希望从源码运行，请遵循以下步骤：

### 1. 克隆项目 (Clone)
```bash
git clone https://github.com/tianwen42/DJI_Batch_LUTer.git
cd DJI_Batch_LUTer
```

### 2. 安装依赖 (Install)
```bash
pip install -r requirements.txt
```

### 3. 初始化 FFmpeg (Setup)
运行自动配置脚本，它会为你下载轻量版 FFmpeg 并放置在正确目录：
```bash
python src/setup_ffmpeg.py
```

### 4. 运行或打包 (Run/Build)
- **直接运行程序**:
  ```bash
  python src/DJI_Batch_LUTer.py
  ```
- **生成独立 EXE (打包)**:
  如果你想生成一个不需要 Python 也能运行的 `.exe` 文件：
  ```bash
  python src/build.py
  ```
  打包完成后，生成的程序将在 `dist/DJI_Batch_LUTer.exe`。

## 🛠️ 使用说明

1. **选择素材**: 将视频放入 `RAW` 目录，或点击按钮手动选择目录。
2. **选择设备与滤镜**:
   - 在“设备”下拉框选择你的相机型号（如 Action 5 Pro）。
   - 在“类型”下拉框选择“还原”或“调色”。
   - 在“滤镜列表”中选择具体的滤镜文件。
   - 也支持点击“手动选择 LUT”来加载任何外部 `.cube` 文件。
3. **设置并发**: 建议显卡加速时设为 1-2，纯 CPU 模式可设为 CPU 核心数。
4. **开始导出**: 点击按钮，等待进度条走完。

## ⚙️ 导出与画质
- **画质选择**: 
  - **原画质**: 硬件加速 150Mbps / CPU CRF 14 (视觉无损，推荐用于专业后期)。
  - **高质量**: 硬件加速 80Mbps / CPU CRF 16 (高保真)。
  - **标准**: 硬件加速 40Mbps / CPU CRF 20 (平衡，兼顾画质与文件大小)。
  - **极速**: 硬件加速 15Mbps / CPU CRF 24 (快速预览)。
- **硬件加速回退**: 如果显卡不支持硬件加速（如 NVENC 开启失败），程序会**自动回退**到 CPU (libx264) 模式，确保转换任务不会中断。

## ⚖️ 导出参数
- **视频编码**: H.264
- **画质控制**: 
  - **原画质**: 硬件加速 150Mbps / CPU CRF 14 (视觉无损)
  - **高质量**: 硬件加速 80Mbps / CPU CRF 16 (高保真)
  - **标准**: 硬件加速 40Mbps / CPU CRF 20 (平衡)
  - **极速**: 硬件加速 15Mbps / CPU CRF 24 (快速)
- **音频**: 原始音轨无损复制 (Copy)
- **像素格式**: 自动处理 10-bit 到 8-bit 的转换以保证兼容性

## ☕ 喝杯咖啡

如果您觉得这个工具对您有所帮助，欢迎赞助作者喝杯奶茶，您的支持是我持续更新的动力！

<img src="doc/alipay_qr.jpg" width="250" alt="支付宝收款码">

## 声明与致谢

### 项目声明
本工具仅作为视频处理辅助工具，不对处理后的视频质量做绝对保证。请在使用前备份好你的原始素材。

### 致谢
- **[FFmpeg](https://ffmpeg.org/)**: 感谢 FFmpeg 团队提供的强大开源视频处理引擎。本工具的核心处理逻辑完全基于 FFmpeg。
- **[DJI (大疆创新)](https://www.dji.com/)**: 感谢大疆提供的专业色彩科学及官方 D-Log M 还原滤镜。

---
*本项目与大疆创新官方无直接关联，仅为个人开发者为提升工作效率而制作。*

## 📝 注意事项
- 如果处理失败，请检查 FFmpeg 路径是否正确。
- 本工具已针对 10-bit 素材做了 8-bit 编码兼容性优化。
- 建议定期将最新的官方 LUT 文件放入 `assets/luts` 目录以获得最佳支持。
- 我只有一个Dji action4，其他系列文件未测试。

